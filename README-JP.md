...
ログインし直してください**。macOS は新しくインストールされた
入力メソッドを、システム設定 > キーボード > 入力ソースの表示・
メニューバー表示・変換エンジンの起動のいずれも、次回ログイン時に
しか反映しません — これは Google 日本語入力など他のサードパーティ製
macOS IME も共通して必要とする一度限りの手順で、`nagi`だけの制約では
ありません。フルの再起動は不要で、ログアウトだけで十分です。`nagi`は
pre-alpha で未署名・未公証
（有償の Apple Developer ID がまだないため）— `.dmg`内の`README.txt`に、
macOS がより頑固に開くのを拒む場合の対処法も含めたフルの手順があります。
...
ください — Nagi が初回起動時に自分でそこへコピーしておくので、
インストール時に別途ドラッグする必要はなく、必要になったときに
そこにあります（Nagi が一度も起動できなかった場合は、`.dmg`の中にも
同じものが予備として入っています）。確認と管理者パスワードの入力の後、
