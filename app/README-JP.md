...
意図的に「アプリのみ」であり、`.pkg`／カスタムインストーラーではない — 署名・公証済みで、アプリだけが入っていて、ユーザーが好きな場所にドラッグできる DMG というのが、大手企業のインストーラーの外側で見ると、個人／小規模チームの macOS 開発者が実際に収斂している標準だから（#30 で議論、[この r/opensource のスレッド](https://www.reddit.com/r/opensource/comments/1ku0zv0/) のご指摘に感謝 — このセクションの以前のバージョンは`.pkg`を指していた）。唯一必要な配慮: Nagi は`/Applications/`ではなく`/Library/Input Methods/`に置かれる必要がある。以前のバージョンの DMG は、通常の Applications エイリアスと同じ発想で、そのフォルダへのシンボリックリンクをドロップ先にすることでこれに対応していた — しかし実機検証（#30 フォローアップ）の結果、Finder のドラッグ&ドロップが管理者パスワードダイアログへエスカレーションするのは固定の許可リスト（`Applications`、`Applications/Utilities`、`Desktop`、いくつかの`Library`サブフォルダ）に限られており、`/Library/Input Methods`を含むそれ以外の場所を指すエイリアス／シンボリックリンクへのドロップはシステムポリシーによってサイレントにブロックされる（エラーもパスワードダイアログも出ず、ただアプリがコピーされない）ことが判明した（[この Apple Developer Forums のスレッド](https://developer.apple.com/forums/thread/712148) 参照）。そのため DMG は現在、代わりにダブルクリックで完結する`Install Nagi.app`を同梱している（`scripts/dmg/Install Nagi.applescript`、`scripts/build-installer.sh`でビルド）: `do shell script ... with administrator privileges`はこの Finder のドラッグポリシーの対象外である別の権限昇格経路のため、回避策を README に書いて誰も読まないまま詰まるより、問題自体を迂回する形にした。Nagi.app の quarantine 属性の剥がしと起動も自動で行う（下記「未署名」参照）— ダブルクリック 1 回と管理者パスワード 1 回でインストールが完結する。`Uninstall Nagi.app`用に 2 回目のドラッグを求めることもない（#33 フォローアップ）: `Nagi.app`が自分自身の中にそのコピーを同梱していて、初回起動時に自分で`/Applications/`へ配置する（`app/Sources/Nagi/UninstallerDeployment.swift`）— 下記の`NagiConverter`や Text Input Source の自己登録と同じ仕組み。`/Applications/`への書き込みは`/Library/Input Methods/`と違って root 権限不要（`root:admin`でグループ書き込み可能、自分でアプリをドラッグしたときに Finder が認証を求めないのと同じ理由）なので、`Install Nagi.app`で既に求められる以上の権限ダイアログは出ない。`.dmg`自体は意図的に`Nagi.app`と`Install Nagi.app`の 2 つだけで、同梱 README も、`Uninstall Nagi.app`のトップレベルの予備コピーも入っていない — 「2 つのファイル、片方をドラッグ、もう片方をクリック」という見た目のシンプルさを、通常ケースで保つため。トレードオフとして: もし`Nagi.app`が一度も起動できなかった場合（例えば`Install Nagi.app`自体が Gatekeeper にブロックされて諦めた場合）、DMG 内に GUI でのアンインストール手段は無い — 残るのは Finder で`/Library/Input Methods/Nagi.app`を手動削除するか、リポジトリを clone した人なら`scripts/uninstall-ime.sh`を使うことのみ。カスタムインストーラー*パッケージ*を不要にできた理由: `Nagi.app`は初回起動時に`SMAppService`経由で自分自身の`NagiConverter` LaunchAgent を登録するようになった（`app/Sources/Nagi/ConverterServiceRegistration.swift`参照）ほか、M4 の #30 フォローアップとして、自分自身を Text Input Source として登録するようにもなった（`app/Sources/Nagi/InputSourceRegistration.swift`、下記「登録されるまで」参照）— もはやインストーラースクリプトがそのどちらも代行する必要はない。ただしどちらも、実際に使えるようになるのは次回ログイン時からで（同じく下記「登録されるまで」参照）、フルの reboot は不要になったものの即時ではない。

**未署名 — このリポジトリの背後に Apple Developer Program のメンバーシップ（Developer ID）は存在しないため、macOS の Gatekeeper が`Install Nagi.app`の初回起動をブロックする。** Control-click → 開く → 開く（ダイアログでもう一度）、または初回のブロック後にシステム設定 → プライバシーとセキュリティ → 「このまま開く」。どちらも「開く」の選択肢が出ない場合（最近の macOS では代わりに「壊れている」と表示され、GUI での回避手段がないことがある — 上記スレッドでも指摘あり、かつ Chromium 系ブラウザでの実ダウンロードで確認済み: 「壊れている」ダイアログの「ゴミ箱に入れる」ボタンはマウント済みの`.dmg`ごとアンマウントしてしまうため、ボリュームがまだマウントされている前提のフォールバックは行き止まりになる）、quarantine 属性はマウント済みボリューム内の何かにではなく、**ダウンロードした`.dmg`ファイル自体**に対して、開き直す前に手動で剥がす: `xattr -cr ~/Downloads/Nagi-<version>.dmg`。`Nagi.app`自身はこの手順を踏む必要がない — `Install Nagi.app`が起動前に自動で quarantine 属性を剥がす（上記参照）。これは同じく個人開発の macOS IME である[SwiftyGyaim](https://github.com/tanabe1478/SwiftyGyaim) が同じ理由で行っているのと同じトレードオフ。署名・公証済み DMG（macSKK/AquaSKK/azooKey-Desktop に合わせる、いずれも#30 で調査済み）はまだ roadmap（M4）上にある — 有償の Developer ID が先に必要。
